...
class Dashboard extends React.Component {
	...
	callTimer() {
		...
		httpPost(urls.start_mission, JSON.stringify(this.state.body), this.props.token).then(
			(result) => {
				this.setStartMissionErrorVisible(false)
	...
	getMissions = () => {
		...
		httpPost(urls.missions, JSON.stringify(this.state.body), this.props.token).then(
			(result) => {
				this.setMissionsErrorVisible(false)
	...
	dashboardStyles() {
		switch (this.props.dashboardState) {
			case 1: {
				return this.props.timer_status ? [styles.drag_container] : [styles.drag_container]
				break
			}
			case 2: {
		...
		return (
			<View style={styles.main_view}>
				...
				<CustomAlert
					...
					decline_btn_handler={() => {
						this.setFinishMissionErrorVisible(!this.state.finishMissionErrorVisible)
					}}
				/>
				<FacebookLogin
					ref='facebookLogin'
						...
						this.state.pickedCashout ? (
							...
							>
								...
								>
									...
									<Text style={styles_top.finishMission_text}>{I18n.t('FOR_TRC')}</Text>
								</View>
								<Text
									style={[styles_top.text_small, this.state.forceCloseHeader && { display: 'none' }]}
								>
							...
							>
								...
								>
									...
									>
										...
										<Text style={styles_top.epc_info}>{I18n.t('FOR_BEING_IN_MALL')}</Text>
										<Text style={this.state.notInMall ? styles_top.epc : styles_top.epc_info}>
											{this.state.notInMall ? I18n.t('NOT_IN_MALL') : I18n.t('TIME_STARTED')}
										</Text>
									</Animated.View>
								</Animated.View>
