...
class Map extends React.Component {
	...
	componentWillReceiveProps = (nextProps) => {
		...
		if (nextProps.navigateToMall) {
			this.moveMapTo(nextProps.selectedMall.lat, nextProps.selectedMall.lng, 0.0058, 0.0058)
	...
	loadCashoutItems = (trc) => {
		...
		httpPost(urls.get_outlet_products, JSON.stringify(body), this.props.token).then(
			(result) => {
				this.setModalVisible(false)
				this.props.loaderState(false)
		...
		return (
			<View style={styles.main_view}>
				<CustomAlert
					...
					first_btn_handler={() => this.setLoginErrorVisible(!this.state.errorVisible)}
					decline_btn_handler={() => this.setLoginErrorVisible(!this.state.errorVisible)}
				/>
				<CustomAlert
				...
				>
					<Button
						style={[styles.state_change_block_btn, styles.state_change_block_btn_left]}
						transparent
