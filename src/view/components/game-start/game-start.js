...
import { View, Text, Image, FlatList, AppState, AsyncStorage } from 'react-native'
...
class GameStart extends React.Component {
	...
	forceRemoveTicker = () => {
		this.props.loaderState(true)
		httpPost(urls.force_remove_ticker, JSON.stringify({}), this.props.token).then(
			(result) => {
				this.setState({ website_visible: false })
				this.props.loaderState(false)
				this.props.getGameInfo(this.props.token, this.props.location.lat, this.props.location.lng)
				...
				{this.props.game_info.no_more_games ? null : (
					<View style={styles.game_title}>
						<Text style={styles.game_title_text}>
							{this.props.game_info.available_game_len}/{this.props.game_info.total_game_len}{' '}
				...
				{this.props.game_status === 'lock' ? (
					<View style={styles.lock_container}>
						...
						<LinearGradient
							...
							style={styles.grad}
						/>
						<Text style={[styles.zifi_text, styles.white_text]}>{I18n.t('GAME.ZIFI.MORE_GAMES')}</Text>
						<Image
							style={[styles.zifi, styles.open_more_game_zifi]}
							source={require('../../../assets/img/zifi/playful.gif')}
						/>
						<Button rounded active={false} block transparent style={styles.go_to_signin} onPress={() => {}}>
							<Text style={styles.lock_visit_text}>{I18n.t('GAME.VISIT_PARTNERS')}</Text>
						</Button>
						<FlatList
						...
						{this.props.selectedMall.active ? (
							<Button rounded block transparent style={styles.go_to_map} onPress={() => this.goToMap()}>
								<Text style={styles.lock_visit_text}>{I18n.t('GAME.VISIT_NEAREST_ONE')}</Text>
								<FastImage
									resizeMode={FastImage.resizeMode.contain}
					...
					<View style={styles.container}>
						<FastImage
							...
							source={{
								uri: this.props.game_info.no_more_games ? ICONS.ZIFI.CLOUD_1 : ICONS.ZIFI.CLOUD_2,
							}}
						/>
