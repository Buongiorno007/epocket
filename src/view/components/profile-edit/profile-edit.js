...
import { Platform, Text, View, KeyboardAvoidingView, BackHandler } from 'react-native'
...
class ProfileEdit extends React.Component {
	...
	SubmitEdit = () => {
		if (!this.state.user.username || this.props.birthday === '' || !this.state.user.photo) {
	...
	render() {
		return (
			<View style={styles.main}>
				...
				{(this.state.modalVisible || this.state.exitVisible || this.state.rejectedRequestModal) && <Blur />}
				<View style={styles.user_edit_header_container}>
					<CustomAlert
						datepicker
						first_btn_title={I18n.t('PROFILE_PAGE.ACCEPT')}
						visible={this.state.datePickerVisible}
						first_btn_handler={() => this.setDatePickerVisible(!this.state.datePickerVisible)}
						decline_btn_handler={() => this.setDatePickerVisible(!this.state.datePickerVisible)}
					/>
					<CustomAlert
						...
						second_btn_handler={() => this.setExitVisible(!this.state.exitVisible)}
						first_btn_handler={() => NavigationService.navigate('Main')}
						decline_btn_handler={() => this.setExitVisible(!this.state.exitVisible)}
					/>
				</View>
				...
				>
					<View style={styles.user_edit_container}>
						...
						<View style={styles.text_container}>
							...
							>
								<Text style={styles.datepicker_button_title}>{I18n.t('PROFILE_PAGE.BIRTHDAY')}</Text>
									...
									<LinearTextGradient
										...
										colors={
											this.state.user.sex === 1
												...
												: [this.props.userColor.black41_09, this.props.userColor.black41_09]
										}
										start={{ x: 0.0, y: 0.0 }}
										end={{ x: 0.7, y: 1.0 }}
										style={styles.title}
				...
				<CustomAlert
					...
					decline_btn_handler={() => this.setRejectVisible(!this.state.rejectedRequestModal)}
				/>
			</View>
		)
	}
