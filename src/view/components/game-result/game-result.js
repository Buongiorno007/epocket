		...
		return (
			<View style={styles.container}>
				...
				<CustomAlert
					...
					first_btn_handler={() => this.setErrorVisible(!this.state.cantPostToInsta)}
					decline_btn_handler={() => this.setErrorVisible(!this.state.cantPostToInsta)}
				/>
				<CustomAlert
				...
				<View style={this.props.navigation.state.params.status === 'success' ? styles.success : styles.failed}>
					<FastImage
						...
						source={{
							uri: this.chooseZifiCloud(this.props.navigation.state.params.status).zifiCloud,
						}}
					/>
					...
					/>
					<Text style={this.chooseResultText(this.props.navigation.state.params.status).style}>
						{this.chooseResultText(this.props.navigation.state.params.status).text}
					</Text>
					{this.props.navigation.state.params.status !== 'success' ? (
					...
					{this.props.navigation.state.params.status !== 'success' ? (
						...
						>
							...
							>
								{this.chooseButtonText(this.props.navigation.state.params.status)}
							</Text>
						</Button>
					)}
					<Button
...
async function requestWriteExternalStoragePermission() {
	try {
		...
		if (granted === PermissionsAndroid.RESULTS.GRANTED) {
			...
			const writeRequest = await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE)
