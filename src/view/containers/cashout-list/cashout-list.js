...
class CashoutList extends React.Component {
						...
						<View style={styles.header_text}>
							...
							<Text style={styles.positions}>
								{section.products.length === 1
									? section.products.length + ' ' + I18n.t('CASHOUT_LIST.POSITIONS_1')
									: section.products.length > 1 && section.products.length <= 4
									? section.products.length + ' ' + I18n.t('CASHOUT_LIST.POSITIONS_2')
									: section.products.length + ' ' + I18n.t('CASHOUT_LIST.POSITIONS_3')}
							</Text>
						</View>
					</View>
					<Animated.Image
	...
	_renderItem = (item) => <CartCard key={item.item.id} cardInfo={item.item} deleteElem={this.deleteElem} />
	keyExtractor = (item) => {
		return item.id
	}
	render = () => {
