					...
					return (
						<View style={[styles.item, !item.amount && styles.hide]} key={i}>
							<View style={styles.one_product}>
								...
								<View style={[styles.data, !item.approve && styles.error, styles.align]}>
									<Text
										style={[
											styles.text,
									...
									>
										{item.amount > 1 && !item.approve
											? `${formatNumber(item.product__price)} x ${item.amount} = ${formatNumber(
													item.product__price * item.amount,
											  )}`
