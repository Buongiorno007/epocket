...
export const getGameInfo = (token, latt, long) => async (dispatch, getState) => {
	dispatch(loaderState(true))
	...
	httpGet(urls.game_get + '?coords=' + latt + '%2C' + long, token).then(
		(result) => {
			...
			if (game.ticker === false && !game.game_set) {
				...
				if (game.brand_partners.lenght % 2 !== 0) game.brand_partners.push({ invisible: true })
		...
		(error) => {
			console.log(error, 'ERRORRERERERER')
			if (error.code === 400) {
				let info = {
					description: '...',
					cost: '0',
